...
public class BloodworldModVariables {
	...
	public static class MapVariables extends WorldSavedData {
		...
		public NBTTagCompound writeToNBT(NBTTagCompound nbt) {
			nbt.setString("MOD_VERSION", MOD_VERSION);
			return nbt;
		}
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				BloodworldMod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(0, this));
			} else {
				BloodworldMod.PACKET_HANDLER.sendToAll(new WorldSavedDataSyncMessage(0, this));
			}
		}
	...
	public static class WorldVariables extends WorldSavedData {
		public static final String DATA_NAME = "bloodworld_worldvars";
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				BloodworldMod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(1, this));
			} else {
				BloodworldMod.PACKET_HANDLER.sendToDimension(new WorldSavedDataSyncMessage(1, this), world.provider.getDimension());
			}
		}
	...
	public static class WorldSavedDataSyncMessageHandler implements IMessageHandler<WorldSavedDataSyncMessage, IMessage> {
		...
		private void syncData(WorldSavedDataSyncMessage message, MessageContext context, World world) {
			if (context.side == Side.SERVER) {
				...
				if (message.type == 0)
					BloodworldMod.PACKET_HANDLER.sendToAll(message);
				else
					BloodworldMod.PACKET_HANDLER.sendToDimension(message, world.provider.getDimension());
			}
			if (message.type == 0) {
